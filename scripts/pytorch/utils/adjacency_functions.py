...
def binary_adjacency_diag_norm_diag(full_processed, nodes_keys):
    """
    Input:
        1. full_processed (from full_data_process.py)
        2. nodes_keys
    ...
    Description:
        Runs normalization on onehot adjacency matrix with self-adjacency
        Re-set diagonal values to 1 after normalization

    Purpose:
        Needed to get binary adjacency matrix
    """
    return set_diagonal(norm_adjacency(set_diagonal(binary_adjacency(full_processed, nodes_keys))))
